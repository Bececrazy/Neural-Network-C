...
class tensor{
	...
	void tensor::show_tensor(){
		printf("(%d,%d):\n", size[0], size[1]);
		for(int i=0; i<size[1] ;i++){ for(int j=0; j<size[0] ;j++){printf("%.1f\t",data[get_index(i,j)]);}
		printf("\n");}printf("\n");}
...
tensor operator*(tensor A, tensor B){
	...
	if(A.size[0] != B.size[1]){ printf("producto . erroneo (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
tensor operator+(tensor A, tensor B){
	...
	else{ if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("adicion erronea: (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
tensor operator-(tensor A, tensor B){
	T_reg[0].relloc(B.size[0],B.size[1]);
	if(A.size[2]==0){ return B; }
	else{ if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("substraccion erronea: (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
		  for(int i=A.size[2]; i--;){T_reg[0].data[i] = A.data[i] - B.data[i];}}
	return T_reg[0];}
					
// Tensor math  ##############################################################//
void operator+=(tensor A, tensor B){
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){printf("adicion erronea: (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
	for(int i=A.size[2]; i--;){A.data[i] += B.data[i];}}
		
void operator-=(tensor A, tensor B){
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){ printf("sustraccion erronea: (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
	for(int i=A.size[2]; i--;){A.data[i] -= B.data[i];}}
			
// Tensor math with point to point ###########################################//
tensor operator>(tensor A, tensor B){
	T_reg[2].relloc(A.size[0],A.size[1]);
	if(A.size[0] != B.size[0] or A.size[1] != B.size[1]){ printf("producto X erronea: (%d,%d) (%d,%d)\n", A.size[0],A.size[1],B.size[0],B.size[1]); exit(1);}
...
//TODO: Image Processing #####################################################//
	...
	tensor decrease_image(tensor A,int new_size){
		if(A.size[2] < pow(new_size,2)){
			printf("error en resize (%d,%d) (%d,%d)",
...
// FIXME: Create a Tensor increase_image(){};
	...
	tensor convolution(tensor A, tensor B){
		...
		if(A.size[2] < B.size[2]){printf("error en la convolution (%d,%d)*(%d,%d)", A.size[0], 	A.size[1], B.size[0],	B.size[1]); exit(1);}
	...
	tensor correlation(tensor A, tensor B){
		I_reg[3].relloc(A.size[0]-B.size[0]+1, A.size[1]-B.size[1]+1);
		if(A.size[2] < B.size[2]){printf("error en la correlation (%d,%d)*(%d,%d)", A.size[0], 	A.size[1], B.size[0],	B.size[1]); exit(1);}
...
class mlp{
	...
	/// neural network ########################### compile functions ///
		...
		void compile(){
			if(topology_size < i_layer){printf("maximo de capas superado %d mlp\n",(i_layer-topology_size));exit(1);}
			else if(topology_size > i_layer){printf("capas faltantes %d mlp\n",(topology_size-i_layer));exit(1);}
...
class rbfnn{
	...
	/// neural network ########################### compile functions ///
		...
		void compile(){
			if(topology_size < i_layer){printf("maximo de capas superado %d rbf\n",(i_layer-topology_size));exit(1);}
			else if(topology_size > i_layer){printf("capas faltantes %d rbf\n",(topology_size-i_layer));exit(1);}
...
class rnn{
	...
	/// neural network ########################### compile functions ///
		...
		void compile(){
			if(topology_size < i_layer){printf("maximo de capas superado %d RNN\n",(i_layer-topology_size));exit(1);}
			else if(topology_size > i_layer){printf("capas faltantes %d RNN\n",(topology_size-i_layer));exit(1);}
