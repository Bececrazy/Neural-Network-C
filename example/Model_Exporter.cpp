...
int main(){
	mlp M(3,0.3,sgd,cross_entropy);
	M.add_layer(4,sigmoid,input_layer);
	M.add_layer(5,sigmoid,hidden_layer);
	M.add_layer(3,sigmoid,output_layer);
	M.compile();
	
	for(int i=500; i--;){ for(int j=0; j<150 ;j++){ M.fit(dataset[j][0],dataset[j][1]);}}
	for(int j=0; j<12 ;j++){M.predict(test[j]);}
