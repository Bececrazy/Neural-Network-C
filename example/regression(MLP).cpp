...
int main(){
	mlp M(3,0.01,sgd,mean_squared);
	M.add_layer(6,sigmoid,input_layer);
	M.add_layer(6,sigmoid,hidden_layer);
	M.add_layer(1,linear,output_layer);
	M.compile();
	
	for(int i=1000; i--;){ for(int j=0; j<30 ;j++){ M.fit(dataset[j][0],dataset[j][1],100);}}
	for(int j=0; j<4 ;j++){M.predict(test[j],100);}
	return 0;}